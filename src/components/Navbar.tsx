...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const handleInstallClick = async () => {
    ...
    if (promptToUse && typeof promptToUse.prompt === 'function') {
      ...
      // If native prompt has not fired yet (e.g. inside iframe), open guide modal
      setIsInstallModalOpen(true);
    }
  };
  ...
  return (
    <header className="sticky top-0 z-40 bg-[#006633] dark:bg-slate-900 text-white shadow-sm transition-colors border-b border-green-800 dark:border-slate-800">
      {/* Address Bar Install Helper Ribbon (shown if embedded in preview frame) */}
      {isInIframe && !dismissIframeBanner && !isInstalled && (
        <div
          id="address-bar-install-helper-banner"
          className="bg-amber-400 dark:bg-amber-950 text-slate-950 dark:text-amber-100 px-4 py-1.5 text-xs font-medium border-b border-amber-500 dark:border-amber-800 flex items-center justify-between gap-2 transition-all shadow-xs"
        >
          <div className="flex items-center space-x-2 truncate">
            <Laptop className="w-4 h-4 text-slate-900 dark:text-amber-400 shrink-0" />
            <span className="truncate">
              <strong>Address Bar 1-Click Install:</strong> Open this app in its own browser tab to activate the <strong>Install icon (💻 or ⊕)</strong> in Chrome & Edge's address bar.
            </span>
          </div>
          <div className="flex items-center space-x-2 shrink-0">
            <button
              id="open-in-tab-address-bar-btn"
              type="button"
              onClick={handleOpenStandalone}
              className="px-2.5 py-0.5 rounded bg-slate-950 hover:bg-slate-800 text-amber-300 text-[11px] font-bold flex items-center space-x-1 cursor-pointer transition-colors shadow-xs"
              title="Open standalone window where address bar install icon appears"
            >
              <ExternalLink className="w-3 h-3 text-amber-400" />
              <span>Open in New Tab</span>
            </button>
            <button
              id="show-install-guide-btn"
              type="button"
              onClick={() => setIsInstallModalOpen(true)}
              className="px-2 py-0.5 rounded bg-amber-500/30 hover:bg-amber-500/50 text-slate-900 dark:text-amber-200 text-[11px] font-semibold cursor-pointer transition-colors"
            >
              Guide
            </button>
            <button
              id="dismiss-address-bar-banner-btn"
              type="button"
              onClick={() => setDismissIframeBanner(true)}
              className="p-1 text-slate-700 dark:text-amber-300 hover:text-slate-950 cursor-pointer"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {/* Universal PWA Install UI Button */}
          {!isInstalled && (
            <button
      ...
      {/* PWA Install Modal with Address Bar Visual Guide */}
      <PwaInstallModal
