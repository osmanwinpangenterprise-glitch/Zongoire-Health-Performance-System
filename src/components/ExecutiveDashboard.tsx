...
import { exportFacilitySpecificReport } from '../utils/exportUtils';
...
export const ExecutiveDashboard: React.FC<ExecutiveDashboardProps> = ({
  ...
  const handleExportSingleFacility = (facilityMetric: CalculatedFacilityMetrics) => {
    const facRecords = monthlyData.filter((d) => d.facilityId === facilityMetric.facilityId);
    exportFacilitySpecificReport(facilityMetric, facRecords, periodLabel);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Data Source Composition Strip */}
      <div className="bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded p-2.5 flex flex-wrap items-center justify-between text-xs gap-2">
        <div className="flex items-center space-x-2">
          <Database className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
          <span className="font-bold text-slate-800 dark:text-slate-200">Data Source Clean Monitoring Status:</span>
          <div className="flex items-center space-x-2 ml-1">
            <span className="bg-emerald-100 dark:bg-emerald-950/80 text-emerald-800 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800 px-2 py-0.5 rounded font-extrabold text-[10px] uppercase flex items-center space-x-1">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-600"></span>
              <span>{monthlyData.filter(d => d.isSample === false || d.dataSource === 'actual').length} Actual DHIMS2 Submissions</span>
            </span>
            <span className="bg-amber-100 dark:bg-amber-950/80 text-amber-800 dark:text-amber-300 border border-amber-300 dark:border-amber-800 px-2 py-0.5 rounded font-extrabold text-[10px] uppercase flex items-center space-x-1">
              <span className="w-1.5 h-1.5 rounded-full bg-amber-500"></span>
              <span>{monthlyData.filter(d => d.isSample !== false && d.dataSource !== 'actual').length} Baseline Sample Records</span>
            </span>
          </div>
        </div>
        <div className="flex items-center space-x-2 text-[11px] text-slate-500 dark:text-slate-400">
          <span>Tip: Upload official DHIMS2 files in the Importer to replace sample baseline records.</span>
        </div>
      </div>
            ...
            <div className="px-4 py-2 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/40">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-200">
                  2026 GHS Sub-District Performance & Credential Matrix
                </h3>
              </div>
              <span className="text-[10px] bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300 px-2 py-0.5 rounded font-bold uppercase border border-emerald-300">
                Period: {periodLabel}
              </span>
            </div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-800/80 text-slate-400 dark:text-slate-400 font-bold text-left border-b border-slate-200 dark:border-slate-700 text-[10px] uppercase tracking-wider">
                    ...
                    <th className="px-3 py-2 text-center">Penta3</th>
                    <th className="px-3 py-2 text-center">Malaria3 '26</th>
                    <th className="px-3 py-2 text-center">HPV1 '26</th>
                    <th className="px-3 py-2 text-center">ANC8+ '26</th>
                    <th className="px-3 py-2 text-center">Skilled Del.</th>
                    <th className="px-3 py-2 text-center">Score</th>
                    <th className="px-3 py-2 text-center">Grade</th>
                    <th className="px-3 py-2 text-center">Credential Code</th>
                    <th className="px-3 py-2 text-center">Export</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {sortedByScore.map((f) => {
                    const code = f.credentialVerificationCode || `GHS-BW-ZSHPMS-2026-${f.facilityId.toUpperCase()}`;
                      ...
                      <tr key={f.facilityId} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50">
                        ...
                        <td className="px-3 py-2 text-center font-mono text-[10px]">
                          <button
                            ...
                            title="Click to copy official GHS credential verification code"
                            className="inline-flex items-center space-x-1 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 px-2 py-1 rounded border border-slate-300 dark:border-slate-700 transition-all cursor-pointer font-semibold"
                          >
                            <span>{code}</span>
